 ...
 *   block_mean=true, is_causal=false, element_pair=nv_float4<float_e2m1>, output=bfloat16
...
static constexpr bool kBlockMean = true;
...
using GmemTiledCopyLambda = SM90_TMA_LOAD;
...
using SmemLayoutLambdaK = decltype(
    make_layout(
        make_shape(Int<32>{}, Int<4>{}, Int<3>{}),
        make_stride(Int<4>{}, Int<1>{}, Int<128>{})
    )
);
...
using LayoutLambdaKV = decltype(
    make_layout(
        make_shape(make_shape(Int<32>{}, int32_t{}), make_shape(Int<4>{}, int32_t{}), make_shape(Int<1>{}, int32_t{})),
        make_stride(make_stride(Int<4>{}, int32_t{}), make_stride(Int<1>{}, int32_t{}), make_stride(Int<0>{}, int32_t{}))
    )
);
